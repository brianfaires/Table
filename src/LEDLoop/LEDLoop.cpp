...
void LEDLoop::SkipTimeForTimers(uint32_t amount) {
  timing.now += amount;
...
void LEDLoop::Setup(uint8_t* pGlobalBrightness)
{
  if(NUM_LEDS == NUM_LEDS_UPPER)
  {
    // 2*2*3*5*6*7
  ...
  else if(NUM_LEDS == NUM_LEDS_INTERIOR)
  {
    // 2*2*2*3*17
  ...
  Gamma.Init(leds, leds_b, leds_5bit_brightness, NUM_LEDS, pGlobalBrightness);
